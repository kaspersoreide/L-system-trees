...
layout (binding = 3) coherent readonly buffer block3
{
    uint n_productions;
...
uniform layout (location = 0) uint iterations;
...
int getProduction(uint c, float randomValue) {
    //return a production for predecessor c
    //if none found returns -1
    float baseValue = 0.0;
    for (int i = 0; i < n_productions; i++) {
        if (productions[i].predecessor == c) {
...
void main() {
    ...
    lastIdx = 0;
    uint leafIdx = 0;
    StringState stringStack[16];
    StringState currentStringState;
    uint stringTop = -1;
    currentStringState.prodIdx = getProduction(string[0]);
    currentStringState.charIdx = 0;
    for (;;) {
        uint c = productions[currentStringState.prodIdxIdx].successor[currentStringState.charIdx];
        switch (c) {
            case 43:    // + turn left
